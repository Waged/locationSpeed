...
public class MainActivity extends AppCompatActivity implements GoogleApiClient.ConnectionCallbacks,
        ...
        switch (requestCode) {
            case PERMISSIONS_REQUEST_ACCESS_FINE_LOCATION: {
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    setButtonsEnabledState(); //  to show again the button
                    startLocationUpdates();
                } else {
                    if (!ActivityCompat.shouldShowRequestPermissionRationale(this, Manifest.permission.ACCESS_FINE_LOCATION)) {
                        mRequestingLocationUpdates = false;
                        Toast.makeText(MainActivity.this, "You've to turn-on location permission to make App Works", Toast.LENGTH_SHORT).show();
                    } else {
    ...
    private void showRationaleDialog() {
        new AlertDialog.Builder(this)
                ...
                .setCancelable(false)
                .setMessage("Application needs location permission to proceed")
                .show();
    }
    ...
    public void onLocationChanged(Location location) {
        ...
        mLastUpdateTime = DateFormat.getTimeInstance().format(new Date());
        if (mRequestingLocationUpdates) {
                //update the textView Here :D
            Toast.makeText(this, getResources().getString(R.string.location_updated_message), Toast.LENGTH_SHORT).show();
           //Here to update the user with the speed
            tvSpeed.setText(((mCurrentLocation.getSpeed()) * 3.6) + " Km/h");
        }
    }
